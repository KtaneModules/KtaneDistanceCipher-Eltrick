...
public class DistanceCipherScript : ModuleScript
{
    ...
    [SerializeField]
    private KMSelectable[] _GenerationZero, _GenerationOne, _GenerationTwo;
    [SerializeField]
    private KMSelectable _SubmitButton;
    [SerializeField]
    private TextMesh[] _DisplayTexts;
    [SerializeField]
	...
	private void PressTile(int x)
    {
		if (_isSolved)
			return;
		ButtonEffect(_module.GetComponent<KMSelectable>(), 1, _Sounds[0]);
		if (x / 3 == 0)
        {
			for (int i = 0; i < _GenerationZero.Length; i++)
	...
	void FixedUpdate()
	{
		...
		_step += _increment;
		_StatusLight.transform.localRotation = Quaternion.Euler((_step * Mathf.PI) / 2f, _step, (_step * Mathf.PI) / 4f);
	}
}
